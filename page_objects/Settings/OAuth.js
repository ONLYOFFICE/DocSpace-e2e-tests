...
export class OAuth extends MainPage {
  constructor(page) {
    super(page);
    this.page = page;
    this.navigateToDevTools = page.getByRole("link", {
    ...
    this.oauthAllowedOriginsInput = page.locator(
      'input[name="allowed_origins"]',
    );
    this.oauthApproveAllowedOriginsInput = page.getByText(
      "https://google.comAdd",
    );
    ...
    this.oauthGenerateToken = page.getByRole("menuitem", {
      name: "Generate token",
    });
    this.oauthGenerateTokenButton = page.getByRole("button", {
      name: "Generate",
    });
    this.oauthRemoveToast = page.getByText(
      "Developer token successfully copied to clipboard",
    );
    this.oauthRevokeToken = page.getByRole("button", { name: "Revoke" });
    this.oauthDisableApplication = page.getByRole("menuitem", {
      name: "Disable",
    });
    this.oauthOKButton = page.getByRole("button", { name: "Ok" });
    this.oauthRemoveToast2 = page.getByText(
      "Application disabled successfully",
    );
    this.oauthEnableApplication = page.getByRole("menuitem", {
      name: "Enable",
    });
