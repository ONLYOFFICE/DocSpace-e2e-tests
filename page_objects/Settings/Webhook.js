...
export class Webhook extends MainPage {
    constructor(page) {
        super(page);
        this.page = page;
        this.developerToolsLink = page.getByRole('link', { name: 'Developer Tools' });
        ...
        this.webhookPayloadUrlInput = page.locator('#create-webhook-payload-url-input');
        this.generateSecretKey = page.locator('form').filter({ hasText: 'This webhook will be assigned' }).getByTestId('link');
        this.createWebhookButton2 = page.getByRole('button', { name: 'Create', exact: true });
        this.webhookName = page.getByText('Autotest');
        this.checkbox = page.getByTestId('checkbox').locator('rect').first();
        this.webhookActionMenu = page.getByTestId('context-menu-button').locator('path').first();
        this.webhookDetails = page.getByRole('menuitem', { name: 'Webhook details' });
        this.webhookRedelivered = page.locator('#sectionScroll').getByRole('img').nth(1);
        ...
        this.deleteForeverButton = this.deleteConfirmMessage.getByText('Delete forever');
