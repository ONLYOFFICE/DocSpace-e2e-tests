...
test.describe('Room Creation API', () => {
  ...
  test('Create form-filling room', async () => {
    ...
    const { room, statusCode } = await roomsApi.createRoom(roomTitle, 'form_filling');
  ...
  test('Create collaboration room', async () => {
    ...
    const { room, statusCode } = await roomsApi.createRoom(roomTitle, 'collaboration');
    expect(statusCode).toBe(200);
    expect(room.title).toBe(roomTitle);
  });
  ...
  test('Create virtual data room', async () => {
    ...
    const { room, statusCode } = await roomsApi.createRoom(roomTitle, 'virtual_data');
    expect(statusCode).toBe(200);
    expect(room.title).toBe(roomTitle);
  });
});
