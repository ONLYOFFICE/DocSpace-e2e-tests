...
test.describe("Profile Smoke Tests", () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    ...
    const portalData = await portalSetup.setupPortalUser();
  });
  ...
  test('edit name', async ({ page }) => {
    ...
    const profileNameLocator = page.locator(`.profile-block-field >> text=${firstName} ${lastName}`);
    await expect(profileNameLocator).toBeVisible();
  });

  test('change password', async ({ page }) => {
    await profilePage.changePassword();
    const changePasswordLocator = page.locator('text=The password change instruction has been sent');
  ...
  test('check notifications tab', async ({ page }) => {
    ...
    const notificationsTabLocator = page.locator('div.notification-container >> text=Actions with files in Rooms');
  ...
  test('check file management tab', async ({ page }) => {
    ...
    const fileManagementTabLocator = page.locator('div.tabs-body >> text=Don\'t ask file name again on creation');
  ...
  test('check interface theme tab', async ({ page }) => {
    ...
    const interfaceThemeTabLocator = page.locator('div.card-header >> text=Dark theme');
  ...
  test('check authorized apps tab', async ({ page }) => {
    ...
    const authorizedAppsTabLocator = page.locator('[data-testid=text] >> text=No authorized apps');
    await expect(authorizedAppsTabLocator).toBeVisible();
  });

  test('login tab', async ({ page }) => {
    const loginTabLocator = page.locator('div.tabs-body >> text=Connect your social networks');
    await expect(loginTabLocator).toBeVisible();
  });

  test('change language', async ({ page }) => {
    await profilePage.changeLanguage();
    const languageChangedLocator = page.locator('data-testid=heading >> text=Mein Profil');
