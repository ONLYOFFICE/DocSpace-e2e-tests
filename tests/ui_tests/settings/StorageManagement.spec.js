...
<<<<<<< HEAD
  ...
  test.skip('Quota Room - временно отключено', async ({ page }) => {
    ...
    await expect(page.locator('text=Room quota has been successfully enabled.')).toHaveText(
      'Room quota has been successfully enabled.',
      { timeout: 10000 }
    );
    await storageManagement.removeToast.click();
    await storageManagement.onOffQuotaRoom.click();
    await expect(page.locator('text=Room quota has been successfully disabled.')).toHaveText(
      'Room quota has been successfully disabled.',
      { timeout: 10000 }
    );
  });
  ...
  test.skip('Quota User - временно отключено', async ({ page }) => {
    ...
    await expect(page.locator('text=User quota has been successfully enabled.')).toHaveText(
      'User quota has been successfully enabled.',
      { timeout: 10000 }
    );
    await storageManagement.removeToast2.click();
    await storageManagement.onOffQuotaUser.click();
    await expect(page.locator('text=User quota has been successfully disabled.')).toHaveText(
      'User quota has been successfully disabled.',
      { timeout: 10000 }
    );
  });
});
