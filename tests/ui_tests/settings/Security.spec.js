...
test.describe("Security tests", () => {
  ...
  test("Security link", async ({ page }) => {
    ...
    await expect(page1).toHaveURL(
      /administration\/docspace-settings.aspx#passwordstrength/,
    );
    ...
    await expect(page2).toHaveURL(
      /administration\/docspace-two-factor-authentication.aspx/,
    );
    ...
    await expect(page3).toHaveURL(
      /administration\/docspace-settings.aspx#TrustedDomain/,
    );
    ...
    await expect(page5).toHaveURL(
      /administration\/configuration.aspx#loginsettings/,
    );
    ...
    await expect(page6).toHaveURL(
      /administration\/docspace-settings.aspx#administratormessage/,
    );
    ...
    await expect(page7).toHaveURL(
      /administration\/docspace-settings.aspx#sessionlifetime/,
    );
  });
  ...
  test.skip("Session lifetime - portal removal method crashes", async ({
    page,
  }) => {
    await security.navigateToSettings();
    await security.navigateToSecurity.click();
