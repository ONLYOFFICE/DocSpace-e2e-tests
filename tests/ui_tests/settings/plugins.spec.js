...
import { PortalLoginPage } from "../../../page_objects/portal_login_page";
    ...
    test('Markdown link', async ({ page }) => {
        ...
        await page1.waitForURL('https://github.com/ONLYOFFICE/docspace-plugins/tree/master/markdown');
        await expect(page1).toHaveURL(/ONLYOFFICE\/docspace-plugins\/tree\/master\/markdown/);
    });
    ...
    test('Draw.io link', async ({ page }) => {
        ...
        const page1 = await plugins.guidePopup();
        await page1.waitForURL('https://github.com/ONLYOFFICE/docspace-plugins/tree/master/draw.io');
        await expect(page1).toHaveURL(/ONLYOFFICE\/docspace-plugins\/tree\/master\/draw\.io/);
    });
    ...
    test('Draw.io change settings', async ({ page }) => {
      ...
      await expect(page.locator('text=Data is saved')).toBeVisible({ timeout: 5000 });
    });
    ...
    test('Speech to text link', async ({ page }) => {
        ...
        const page1 = await plugins.guidePopup();
        await page1.waitForURL('https://github.com/ONLYOFFICE/docspace-plugins/tree/master/speech-to-text');
        await expect(page1).toHaveURL(/ONLYOFFICE\/docspace-plugins\/tree\/master\/speech\-to\-text/);
    });
    ...
    test('Pdf converter link', async ({ page }) => {
        ...
        const page1 = await plugins.guidePopup();
        await page1.waitForURL('https://github.com/ONLYOFFICE/docspace-plugins/tree/master/pdf-converter');
        await expect(page1).toHaveURL(/ONLYOFFICE\/docspace-plugins\/tree\/master\/pdf\-converter/);
    });
    ...
    test('Markdown enable', async ({ page }) => {
        ...
        const responsePromise = page.waitForResponse(
          response => 
            response.url() === `https://${portalDomain}/api/2.0/settings/webplugins/markdown` &&
      ...
      test('Draw.io enable', async ({ page }) => {
        ...
        const responsePromise = page.waitForResponse(
          response => 
            response.url() === `https://${portalDomain}/api/2.0/settings/webplugins/draw.io` &&
            response.request().method() === 'PUT'
        );
        await plugins.navigateToSettings();
        await plugins.navigateToPlugins();
      ...
      test('Speech to text add key', async ({ page }) => {
        ...
        await expect(page.locator('text=Token is saved')).toBeVisible({ timeout: 5000 });
      });
      ...
      test('Speech to text enable', async ({ page }) => {
        ...
        const responsePromise = page.waitForResponse(
          response => 
            response.url() === `https://${portalDomain}/api/2.0/settings/webplugins/speech-to-text` &&
            response.request().method() === 'PUT'
        );
        await plugins.navigateToSettings();
        await plugins.navigateToPlugins();
      ...
      test('Pdf converter enable', async ({ page }) => {
        ...
        const responsePromise = page.waitForResponse(
          response => 
            response.url() === `https://${portalDomain}/api/2.0/settings/webplugins/pdf-converter` &&
            response.request().method() === 'PUT'
        );
        await plugins.navigateToSettings();
        await plugins.navigateToPlugins();
