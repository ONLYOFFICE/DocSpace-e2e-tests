...
test.describe("OAuth tests", () => {
  ...
  test("OAuth token create/revoke", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Developer token successfully copied to clipboard"),
    ).toHaveText("Developer token successfully copied to clipboard", {
      timeout: 10000,
    });
  ...
  test("Enable disable", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Application disabled successfully"),
    ).toHaveText("Application disabled successfully", { timeout: 10000 });
    await oAuth.oauthRemoveToast2.click();
    await oAuth.activationOAuthApplication();
    await expect(
      page.locator("text=Application enabled successfully"),
    ).toHaveText("Application enabled successfully", { timeout: 10000 });
  });
  ...
  test("OAuth delete", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Application deleted successfully"),
    ).toHaveText("Application deleted successfully", { timeout: 10000 });
  });
  ...
  test("OAuth link", async ({ page }) => {
    ...
    await expect(page1).toHaveURL(
      /administration\/docspace-settings.aspx#oauth/,
    );
  });
});
