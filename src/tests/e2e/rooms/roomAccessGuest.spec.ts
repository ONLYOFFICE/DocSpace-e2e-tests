...
// Room created by owner - Guest is invited to it
const INVITED_ROOM = "OwnerRoom";

test.describe("Rooms - Guest access", () => {
  let myRooms: MyRooms;
  let login: Login;
  ...
  test.beforeEach(async ({ page, api, apiSdk }) => {
    ...
    const { userData, response: guestResponse } = await apiSdk.profiles.addMember("owner", "Guest");
  ...
  test("Guest sees invited room", async () => {
    await myRooms.roomsTable.checkRowExist(INVITED_ROOM);
  });

  test("Guest has Pin to top option for invited room", async () => {
  ...
  test("Guest does NOT have Edit room option for invited room", async () => {
    await myRooms.roomsTable.openContextMenu(INVITED_ROOM);
  ...
  test("Guest does NOT have Move to archive option for invited room", async () => {
    await myRooms.roomsTable.openContextMenu(INVITED_ROOM);
    // Anchor: confirm menu is open
    await expect(
      myRooms.roomsTable.contextMenu.getItemLocator(roomContextMenuOption.pinToTop),
  ...
  test.describe("Inside invited room", () => {
    ...
    test("Guest can view Contacts tab in invited room", async () => {
      await myRooms.infoPanel.open();
      await myRooms.infoPanel.openTab("Contacts");
      await myRooms.infoPanel.checkAccessesExist();
    });

    test("Guest can view History tab in invited room", async ({ page }) => {
      await myRooms.infoPanel.open();
    ...
    test("Guest can view Details tab in invited room", async ({ page }) => {
      await myRooms.infoPanel.open();
      await myRooms.infoPanel.openTab("Details");
      await expect(page.getByTestId("info_details_tab")).toBeVisible();
    });

    test("Guest cannot manage members in invited room", async () => {
      await myRooms.infoPanel.open();
      await myRooms.infoPanel.openTab("Contacts");
