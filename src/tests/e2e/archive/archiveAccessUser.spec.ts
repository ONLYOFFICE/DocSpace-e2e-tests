...
// Room created by owner - User is NOT invited (should not appear in archive)
const OWNER_ROOM = "OwnerArchivedRoom";
// Room created by owner - User is invited as editor
const INVITED_ROOM = "InvitedArchivedRoom";

test.describe("Archive - User access", () => {
  ...
  test("User does NOT see archived rooms they were not invited to", async ({
    page,
  }) => {
    await expect(
  ...
  test("User sees invited archived room", async () => {
    await myArchive.archiveTable.checkRowExist(INVITED_ROOM);
  });

  test("User can view info panel for invited archived room", async ({
    page,
  }) => {
