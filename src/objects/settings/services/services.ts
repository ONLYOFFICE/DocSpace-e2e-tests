...
class Services extends BasePage {
  ...
  get backupSwitchModal() {
    return this.page.getByTestId('service-backup-toggle-button').getByTestId('toggle-button-icon');
  }
  ...
  async checkServicesRendered() {
    await expect(this.page.getByTestId('storage_service_total_size')).toContainText('Additional disk storage');
  }
