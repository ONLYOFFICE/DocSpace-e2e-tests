...
class InfoPanel {
  ...
  private get noItemText() {
    return this.page
      .locator(NO_ITEM_TEXT)
      .filter({ hasText: "See file and folder details here" });
  }
  ...
  async openTab(tabName: TInfoPanelTabs) {
    return test.step("Open tab", async () => {
      const tab = this.page.getByTestId(getTabInfo(tabName).testId);
  ...
  async createFirstSharedLink() {
    return test.step("Create first shared link", async () => {
      ...
      const createAndCopy =
        this.sharedLinksWrapper.getByText("Create and copy");
